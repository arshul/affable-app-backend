...
apiUrl = "https://api.graph.cool/simple/v1/cjsr59a4t1a2l0135oyxt8how"

def create_engagement(engagement):
  mut = """mutation CreateEngagement($likesRatio: Float!, $commentsRatio: Float!) {
      createEngagementData(
        avgLikesRatio: $likesRatio
        avgCommentsRatio: $commentsRatio
      ) {
        id
      }
    }"""
  engmtId = requests.post(
    apiUrl,
    json={
      'query':mut,
      'variables':{
        'likesRatio':engagement.get('avgLikesRatio'),
        'commentsRatio':engagement.get('avgCommentsRatio')
      },
    }
  ).json().get('data').get('createEngagementData').get('id')
  return engmtId
def create_stats(stats,eid):
  mut = """
  mutation CreateStat($interests: [String!], $followerCount: Int, $eid: ID) {
    createStatsData(
  ...
  ).json().get('data').get('createStatsData').get('id')
  return statId
def create_influencer(influencer,statsId):
  mut = """mutation CreateInfluencer(
    $username: String!
    $fullname: String
    $picture: String!
    $biography: String
  ...
  infId = requests.post(
    ...
    json={
      ...
      'variables':{
        ...
        'followerCount':influencer.get('followerCount'),
        'statsId':statsId,
      },
    }
  ).json().get('data').get('createInfluencer').get('id')
  return infId


for d in data:
  try:
    print(d.get('fullName'))
    engmtId = create_engagement(d.get('stats').get('engagement'))
    statsId = create_stats(d.get('stats'),engmtId)
    create_influencer(d,statsId)
